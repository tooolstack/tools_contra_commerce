...
function StackedShell({ inputs, result }: { inputs: React.ReactNode; result: React.ReactNode }) {
  return <div className="grid min-w-0 grid-cols-1 gap-5"><section className="min-w-0 space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">{inputs}</section><section className="min-w-0 space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">{result}</section></div>;
}
...
function Teleprompter() {
  const [text,setText]=useState('Welcome. Paste your script here, then start the teleprompter.'); const [speed,setSpeed]=useState('35'); const [notice,setNotice]=useState(''); const screen=useRef<HTMLDivElement>(null);
  ...
  const copyScript=async()=>{await navigator.clipboard.writeText(text);setNotice('Script copied—paste it into any writing, recording or collaboration platform.');setTimeout(()=>setNotice(''),2200)};
  const downloadScript=()=>{const blob=new Blob([text],{type:'text/plain;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download='teleprompter-script.txt';link.click();setTimeout(()=>URL.revokeObjectURL(url),1500);setNotice('Text file downloaded.');setTimeout(()=>setNotice(''),2200)};
  const printScript=()=>{const popup=window.open('','_blank','width=900,height=800');if(!popup){setNotice('Allow pop-ups to print or save the script as PDF.');return}const safe=text.replace(/[&<>"']/g,(character)=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[character]||character));popup.document.write(`<!doctype html><html><head><title>Teleprompter script</title><style>@page{margin:20mm}body{font-family:Arial,sans-serif;font-size:16px;line-height:1.7;text-align:justify;white-space:pre-wrap;color:#111}p{text-align-last:left}</style></head><body><p>${safe}</p><script>addEventListener('load',()=>print())</script></body></html>`);popup.document.close()};
  const importScript=(file?:File)=>{if(!file)return;if(file.type&&file.type!=='text/plain'){setNotice('Choose a plain .txt file.');return}const reader=new FileReader();reader.onload=()=>{setText(String(reader.result||''));setNotice('Text file imported.');setTimeout(()=>setNotice(''),2200)};reader.readAsText(file)};
  return <div className="space-y-4"><Area label="Script" value={text} onChange={setText} rows={8}/><div className="flex flex-wrap items-end gap-3"><Field label="Scroll speed" value={speed} onChange={setSpeed}/><button className={`${buttonClass} h-[42px] shrink-0`} onClick={start}>Start full screen</button></div><section className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-gray-900">Use this script on another platform</h3><p className="mt-1 text-xs leading-5 text-gray-500">Copy it into another app, download a reusable text file, or open the print dialog and choose Save as PDF.</p></div><div className="flex flex-wrap gap-2"><button className={buttonClass} disabled={!text.trim()} onClick={copyScript}>Copy script</button><button className="rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 hover:bg-gray-100 disabled:opacity-50" disabled={!text.trim()} onClick={downloadScript}>Download .txt</button><button className="rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 hover:bg-gray-100 disabled:opacity-50" disabled={!text.trim()} onClick={printScript}>Print / Save PDF</button><label className="cursor-pointer rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-900 hover:bg-gray-100">Import .txt<input type="file" accept="text/plain,.txt" className="hidden" onChange={(event)=>importScript(event.target.files?.[0])}/></label></div></div>{notice&&<p role="status" className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-800">{notice}</p>}</section><div ref={screen} style={{textAlign:'justify',textAlignLast:'left'}} className="h-[460px] overflow-y-auto whitespace-pre-wrap bg-black px-[10vw] py-[35vh] text-4xl font-semibold leading-relaxed text-white">{text}</div></div>;
}
